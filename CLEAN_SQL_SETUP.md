# 🍺 DrunkSafe Database Setup - Clean SQL Code

## ⚠️ IMPORTANT: Copy ONLY the SQL code below (not the markdown)
...
Copy this block and paste into Supabase SQL Editor:
...
### Step 4: Create Automatic Order Processing
Copy this block and paste into Supabase SQL Editor:
...
SELECT 'Tables created:' as test_type, COUNT(*) as count
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('user_tolerance_profiles', 'drink_consumption_log', 'bac_calculations');

SELECT 'Functions created:' as test_type, COUNT(*) as count
...
1. **Copy ONLY the SQL code** from each step above (not the markdown formatting)
2. **Paste into your Supabase SQL Editor**
3. **Click "RUN"** for each step
4. **Wait for "Success. No rows returned"** before moving to the next step

**Do NOT copy the markdown code blocks (```) - only copy the SQL code inside them!**