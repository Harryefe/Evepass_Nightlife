  ...
  return Sentry.startSpan(
    ...
    async (span) => {
      try {
        ...
        const textResponse = generateAIResponse(message, conversationHistory);
...
function generateAIResponse(userMessage: string, conversationHistory: any[] = []): string {
  ...
  if (isFirstMessage || input.includes('hello') || input.includes('hi ') || input.includes('hey')) {
    const greetings = [
      "Hi there! I'm Eve, your AI nightlife assistant. I'm here to help you discover amazing venues, plan your perfect night out, and keep you safe with DrunkSafe™. What can I help you with tonight?",
      "Hey! Welcome to Evepass! I'm Eve, and I'm excited to help you explore the best nightlife experiences. Whether you want to find venues, book tables, or get safety tips, I've got you covered!",
      "Hello! I'm Eve, your personal nightlife companion. I can help you discover venues, manage your night out, track your safety with DrunkSafe™, and so much more. What sounds good to you?"
    ];
  ...
  if (input.includes('safe') || input.includes('safety') || input.includes('drunk') || input.includes('bac') || input.includes('alcohol')) {
    return "Safety is my absolute top priority! DrunkSafe™ is your personal safety companion that monitors your alcohol consumption, calculates your BAC in real-time, and provides personalized safety recommendations. I can set up emergency contacts, track your location, help you get home safely, and even remind you to drink water. Your safety profile shows you're doing great - want me to activate any additional safety features?";
  }
  
  // Social features
  if (input.includes('friends') || input.includes('social') || input.includes('meet') || input.includes('share') || input.includes('bottle')) {
    return "The social side of nightlife is amazing! You can use our 'Share a Bottle' feature to split costs and meet new people, or send song requests directly to DJs at venues. I can help you connect with other party-goers, coordinate group bookings, and even find people with similar music tastes. Want to explore our social features?";
  }
  
  // Booking and reservations
  if (input.includes('book') || input.includes('table') || input.includes('reservation') || input.includes('reserve')) {
    return "I can definitely help you secure the perfect table! I have real-time availability for VIP areas, regular tables, and bar spots at top venues. Most places offer instant booking, and I can even negotiate group discounts for larger parties. Which venue interests you, and how many people will be joining?";
  }
  
  // Music and entertainment
  if (input.includes('music') || input.includes('dj') || input.includes('song') || input.includes('dance') || input.includes('genre')) {
    return "Music makes the night! I can help you find venues that match your musical taste - whether you're into house, techno, hip-hop, or live bands. You can even request songs directly through the app, and I'll send them to the DJ. What kind of vibe are you feeling tonight?";
  }
  ...
  if (input.includes('uber') || input.includes('taxi') || input.includes('ride') || input.includes('home') || input.includes('transport')) {
    return "Getting home safely is crucial! I can book rides for you through Uber, Bolt, or traditional black cabs. I'll also remind you about Night Tube schedules and can share your location with trusted contacts. When you're ready to head home, just let me know and I'll sort out the safest, quickest option for you.";
  }
  
  // Food and late night eats
  if (input.includes('food') || input.includes('eat') || input.includes('hungry') || input.includes('restaurant')) {
    return "Late-night munchies are real! I know all the best spots for post-club food - from 24-hour kebab shops to upscale late-night restaurants. Eating while drinking also helps slow alcohol absorption, which is great for your DrunkSafe™ profile. Want me to find some food options near your current location?";
  }
  
  // Help and features
  if (input.includes('help') || input.includes('what can you') || input.includes('features') || input.includes('do')) {
    return "I'm your complete nightlife assistant! I can help you discover venues, book tables, manage your budget, plan your entire night, keep you safe with DrunkSafe™, connect you with other party-goers, request songs, order rides, find food, and so much more. I'm here 24/7 to make sure you have the best possible night out. What would you like to explore first?";
  }
  
  // Default helpful response
  const defaultResponses = [
    "I'm here to make your night out amazing! I can help with venue recommendations, table bookings, safety monitoring, budget tracking, and connecting with other party-goers. What aspect of your night would you like to focus on?",
    "That's interesting! As your nightlife AI assistant, I can help you with venues, safety, planning, social features, and much more. Is there something specific about your night out I can help you with?",
    "I'm Eve, your AI nightlife companion! I specialize in venue discovery, safety monitoring with DrunkSafe™, night planning, and social features. How can I enhance your nightlife experience tonight?"
  ];
