...
CREATE TABLE IF NOT EXISTS bottle_shares (
  ...
  cost_per_person decimal(10,2), -- Removed GENERATED ALWAYS AS clause
...
-- Function to calculate cost per person for a bottle share
CREATE OR REPLACE FUNCTION calculate_bottle_share_cost()
RETURNS TRIGGER AS $$
DECLARE
  bottle_price decimal(10,2);
BEGIN
  -- Get the bottle price
  SELECT price INTO bottle_price
  FROM menu_items
  WHERE id = NEW.bottle_id;
  
  -- Calculate cost per person
  IF NEW.max_participants > 0 AND bottle_price IS NOT NULL THEN
    NEW.cost_per_person := bottle_price / NEW.max_participants;
  ELSE
    NEW.cost_per_person := 0;
  END IF;
...
-- Create trigger to automatically calculate cost_per_person
CREATE TRIGGER trigger_calculate_bottle_share_cost
  BEFORE INSERT OR UPDATE ON bottle_shares
  FOR EACH ROW
  EXECUTE FUNCTION calculate_bottle_share_cost();
...
AND routine_name IN ('join_bottle_share', 'update_song_request_status', 'get_venue_bottle_shares', 'calculate_bottle_share_cost');
...
SELECT '🎉 SOCIAL FEATURES DATABASE SETUP COMPLETE! 🎉' as message,
       ...
       '✅ Cost calculation automated with triggers' as feature_4,
